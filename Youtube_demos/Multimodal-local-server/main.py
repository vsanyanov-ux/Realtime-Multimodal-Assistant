...
class QwenMultimodalProcessor:
    ...
    def __init__(self):
        ...
        if torch.cuda.is_available():
            ...
            logger.info(f"Loading {model_id} on CPU without quantization (using bfloat16 to save memory)...")
            self.model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
                model_id,
...
async def handle_client(websocket):
    ...
    try:
        ...
        async def receive_audio_and_images():
            logger.info("Started receive_audio_and_images loop")
            async for message in websocket:
                try:
                    ...
                    if "realtime_input" in data:
                        for chunk in data["realtime_input"]["media_chunks"]:
                            if chunk["mime_type"] == "audio/pcm":
                                ...
                                if len(detector.audio_buffer) % 50 == 0:
                                    logger.debug(f"Received audio buffer {len(detector.audio_buffer)}")
                    ...
                    if "image" in data and not detector.tts_playing:
                        ...
                        await qwen_processor.set_image(image_data)
                        logger.debug("Received standalone image")
